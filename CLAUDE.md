...
- Nothing in `private` or `pgmq` is ever granted to `anon` or `authenticated`. Gates 6 and 7 check this; gate 3 only sees `public`. `cron` is guarded differently: its objects are owned by `supabase_admin` and arrive with `PUBLIC` grants a migration cannot revoke, so gate 8 checks the one thing a migration can get wrong about those grants — that no client role holds `usage` on the schema (§9.7).
- The two unlogged `private` tables, `rate_limit_hits` and `discover_cache`, are reached by `security invoker` functions running under the Edge Function's secret key, which resolves to `service_role`. Both grant it `select, insert, update` and not `delete` (§12.2), and each function carries `grant execute ... to service_role` of its own: the mandatory revoke from `public` takes away what the defaults gave, so the grant back is required, not optional (§5.1). §12.2's 5-minute counter cleanup is scheduled by a migration of its own, after the table's, and runs as `postgres` — it needs no grant.
- The two retention tables, `private.preserved_content` and `private.retained_registrations`, are written only inside `security definer` functions owned by the table's owner, placed in `public` so gate 4 looks at them. On the tables themselves `service_role` holds nothing; what it holds is `execute` on the two functions a worker calls — the purge and the account-deletion function (§5.5) — while the snapshot path is a staff `admin_*` function granted to `authenticated` only. Each table has its own reason (§11.6): a conditional purge of `retained_registrations` needs `select` on the column it filters, which would falsify "no role may read it", while `preserved_content`'s snapshot write happens inside a staff `admin_*` call made by `authenticated`, which gate 7 forbids from writing `private` at all. The cost is a path CI cannot inspect — gate 3 skips `admin\_%`, and gate 4 only checks that `search_path` is pinned, never what a body does — so in those bodies the first-line role check, the ban on dynamic SQL (§9.1) and §9.8's never-log list are the only controls, and neither function may return or log `phone` or `email` (§9.8).
- `private` tables enable RLS and ship no policy — every role that can hold anything on them bypasses RLS, `service_role` and `postgres` alike. The proof is an allow test whose negative is exhaustive in **both** dimensions, because `grant all` is eight privileges and a column grant is invisible to `has_table_privilege`: assert that false over every privilege the flow does not need, and `has_any_column_privilege('service_role', '<table>', 'REFERENCES')` false — `REFERENCES` because it is the one column-grantable privilege no flow needs, so the column half stays meaningful even where a legitimate table grant makes every other column privilege true. That pairing is §9.7's own lesson about column grants, applied to the one role no gate measures. `usage on schema private` comes from a migration of its own, landed ahead of the first `private` table rather than hung on it, so reverting a table cannot strip it from the rest.
